...
export default class Event {
    ...
    setServer(eventServer = false, healthcheck = 'healthcheck') {
        if (!eventServer)
            return this.stabilaWeb.eventServer = false;

        if (utils.isString(eventServer))
            eventServer = new providers.HttpProvider(eventServer);

        if (!this.stabilaWeb.isValidProvider(eventServer))
            throw new Error('Invalid event server provided');

        this.stabilaWeb.eventServer = eventServer;
        this.stabilaWeb.eventServer.isConnected = () => this.stabilaWeb.eventServer.request(healthcheck).then(() => true).catch(() => false);
    }
    ...
    getEventsByContractAddress(contractAddress = false, options = {}, callback = false) {
        ...
        if (contractAddress)
            routeParams.push(this.stabilaWeb.address.fromHex(contractAddress));
        ...
        return this.stabilaWeb.eventServer.request(`event/contract/${routeParams.join('/')}?${querystring.stringify(qs)}`).then((data = false) => {
            if (!data)
    ...
    getEventsByTransactionID(transactionID = false, options = {}, callback = false) {
        ...
        return this.stabilaWeb.eventServer.request(`event/transaction/${transactionID}`).then((data = false) => {
            if (!data)
                return callback('Unknown error occurred');
