...
const MAX_FEE_LIMIT = 1_000_000;
export default class SideChain {
    constructor(sideOptions, StabilaWeb = false, mainchain = false, privateKey = false) {
        this.mainchain = mainchain;
        const { fullHost, fullNode, solidityNode, eventServer, mainGatewayAddress, sideGatewayAddress, sideChainId } = sideOptions;
        this.sidechain = new StabilaWeb(fullHost || fullNode, fullHost || solidityNode, fullHost || eventServer, privateKey);
        ...
        const self = this;
        this.sidechain.stb.sign = (...args) => {
            return self.sign(...args);
        };
        this.sidechain.stb.multiSign = (...args) => {
    ...
    async multiSign(transaction = false, privateKey = this.sidechain.defaultPrivateKey, permissionId = false, callback = false) {
        ...
        if (!transaction.raw_data.contract[0].Permission_id && permissionId > 0) {
            ...
            const signWeight = await this.sidechain.stb.getSignWeight(transaction, permissionId);
    ...
    async sign(transaction = false, privateKey = this.sidechain.defaultPrivateKey, useStabilaHeader = true, multisig = false, callback = false) {
        if (this.utils.isFunction(multisig)) {
            callback = multisig;
            multisig = false;
        }

        if (this.utils.isFunction(useStabilaHeader)) {
        ...
        if (this.utils.isString(transaction)) {
            ...
            try {
                const signatureHex = this.sidechain.stb.signString(transaction, privateKey, useStabilaHeader);
    ...
    */
    async depositStb(
        ...
        if (!callback) {
            return this.injectPromise(this.depositStb, callValue, depositFee, feeLimit, options, privateKey);
        }
        ...
        try {
            ...
            const result = await contractInstance.depositSTB().send(options, privateKey);
    ...
    async mappingTrc(
        ...
        if (!callback) {
            return this.injectPromise(this.mappingTrc, stbHash, mappingFee, feeLimit, functionSelector, options, privateKey);
        }
        if (this.validator.notValid([
            {
                name: 'stbHash',
        ...
        stbHash = stbHash.startsWith('0x') ? stbHash : ('0x' + stbHash);
        options = {
            feeLimit,
            ...options,
        ...
        try {
            ...
            if (functionSelector === 'mappingTRC20') {
                result = await contractInstance.mappingTRC20(stbHash).send(options, privateKey);
            } else if (functionSelector === 'mappingTRC721') {
                result = await contractInstance.mappingTRC721(stbHash).send(options, privateKey);
            } else {
    ...
    /**
     * withdraw stb from sidechain to mainchain
     */
    async withdrawStb(
        callValue,
        withdrawFee,
        ...
        if (!callback) {
            return this.injectPromise(this.withdrawStb, callValue, withdrawFee, feeLimit, options, privateKey);
        }
        if (this.validator.notValid([
            {
                name: 'callValue',
        ...
        try {
            ...
            const result = await contractInstance.withdrawSTB().send(options, privateKey);
            return callback(null, result);
        } catch (ex) {
            return callback(ex);
        }
    ...
    async withdrawTrc(
        ...
        try {
            ...
            const signedTransaction = await this.sidechain.stb.sign(transaction.transaction, privateKey);
            ...
            const broadcast = await this.sidechain.stb.sendRawTransaction(signedTransaction);
            if (broadcast.code) {
                const err = {
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.sidechain.stb.getTransactionInfo(signedTransaction.txID);
    ...
    async injectFund(
        ...
        try {
            ...
            const signedTransaction = await this.sidechain.stb.sign(transaction, privateKey);
