...
const FEE_LIMIT = 1500000;

export default class StabilaWeb extends EventEmitter {
    ...
    constructor(options = false,
        ...
        [
            ...
            'toUnit', 'fromUnit', 'toBigNumber', 'isAddress',
            'createAccount', 'address', 'version'
        ].forEach(key => {
            this[key] = StabilaWeb[key];
        });
        // for sidechain
        if (typeof sideOptions === 'object' && (sideOptions.fullNode || sideOptions.fullHost)) {
            this.sidechain = new SideChain(sideOptions, StabilaWeb, this, privateKey);
        } else {
    ...
    async getFullnodeVersion() {
        try {
            const nodeInfo = await this.stb.getNodeInfo()
    ...
    static toHex(val) {
        if (utils.isBoolean(val))
            return StabilaWeb.fromDecimal(+val);

        if (utils.isBigNumber(val))
            return StabilaWeb.fromDecimal(val);

        if (typeof val === 'object')
            return StabilaWeb.fromUtf8(JSON.stringify(val));
        ...
        if (utils.isString(val)) {
            ...
            if ((!isFinite(val)) || /^\s*$/.test(val))
                return StabilaWeb.fromUtf8(val);
        }

        let result = StabilaWeb.fromDecimal(val);
        if (result === '0xNaN') {
    ...
    static toDecimal(value) {
        return StabilaWeb.toBigNumber(value).toNumber();
    }

    static fromDecimal(value) {
        const number = StabilaWeb.toBigNumber(value);
    ...
    static fromUnit(unit) {
        const stb = StabilaWeb.toBigNumber(unit).div(1_000_000);
        return utils.isBigNumber(unit) ? stb : stb.toString(10);
    }

    static toUnit(stb) {
        const unit = StabilaWeb.toBigNumber(stb).times(1_000_000);
        return utils.isBigNumber(stb) ? unit : unit.toString(10);
    }
