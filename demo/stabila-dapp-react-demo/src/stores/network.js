...
import { stabilascanTX } from '../utils/helper';
import Tip from '../components/Tip';
import { getStbBalance, getTransactionInfo } from '../utils/blockchain';
export default class NetworkStore {
  @observable stabilaWeb = false;
  @observable defaultAccount = null; // current login stabila account address
  ...
  getDescription = (type, item, text) => {
    ...
    return (
      <div className={'trans-notify'}>
        <span>{stabilascanTX(intl.get('view_on_stabilascan'), tx)}</span>
        {type === 3 ? (
  ...
  //判断是否有钱包和登录
  checkLogin = () => {
    if (!this.stabilaWeb || !this.stabilaWeb.defaultAddress.base58) {
      return false;
    }
  ...
  initStabilaClickWallet = async (cb, cbn) => {
    try {
      let timeCount = 0;
      const self = this;
      const tmpTimer1 = setInterval(() => {
        timeCount++;
        if (timeCount > Config.stabilaClickTime) {
          self.isConnected = false;
          cbn && cbn();
          clearInterval(tmpTimer1);
        }
        if (window.stabilaWeb && window.stabilaWeb.ready) {
          if (process.env.REACT_APP_ENV === 'test') {
            window.stabilaWeb.setFullNode('https://api.nileex.io');
          }
          self.stabilaWeb = window.stabilaWeb;
          self.defaultAccount = self.stabilaWeb.defaultAddress.base58;
