...
export const toBigNumber = stabilaWeb.toBigNumber;

// export const BigNumber = stabilaWeb.BigNumber;
export const BigNumber = bigNumber;

export const toDecimal = stabilaWeb.toDecimal;

export const getStbBalance = address => {
  return stabilaWeb.stb.getBalance(address);
};
...
export const fromHex = hexString => {
  return stabilaWeb.address.fromHex(hexString.replace('/^0x/', '41'));
};

export const addressToHex = addr => {
  return stabilaWeb.address.toHex(addr);
};

export const isAddress = address => {
  return stabilaWeb.isAddress(address);
};

export const stabilascanAddress = (text, address) => {
  return (
    <a
      className="typo-text-link"
      href={`${Config.stabilascanUrl}/address/${address}`}
      target="_blank"
...
export const stabilascanTX = (text, tx) => {
  return (
    <a
      className="typo-text-link"
      href={`${Config.stabilascanUrl}/transaction/${tx}`}
      target="_blank"
      rel="noopener noreferrer"
    >
      {text}
...
export const copyToClipboard = (e, disBottom = '5px', p = false) => {
  ...
  if (stabilaWeb.BigNumber.isBigNumber(value)) {
